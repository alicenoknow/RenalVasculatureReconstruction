...
def get_starting_points(img):
    ...
    starting_points = peak_local_max(img, min_distance=20)
...
def square_full(array, point, distance):
    shape = array.shape

    min_x = max(0, point[0] - distance)
    max_x = min(shape[0] - 1, point[0] + distance)
    min_y = max(0, point[1] - distance)
    max_y = min(shape[1] - 1, point[1] + distance)
    min_z = max(0, point[2] - distance)
    max_z = min(shape[2] - 1, point[2] + distance)
...
def get_vessel_graph(seg_img, multiplier=1, use_cube=False):
...
#     print("Starting points: ", len(starting_point_stack))
    while starting_point_stack:
        ...
        while points_to_examine:
            ...
            if use_cube:
                visited = square_full(visited, point, radius)
            else:
